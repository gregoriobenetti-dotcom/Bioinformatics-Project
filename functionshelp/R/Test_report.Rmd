...
▶️ Input: bulk expression counts + sample metadata

▶️ Output: mean & median expression for filtered treated genes and per-condition gene means
...
▶️ Input: bulk expression counts

▶️ Output: same table + log2 expression column + binary high-expression flag
...
▶️ Output: joined dataset + validated index speed improvement (no returned summary table)
...
▶️ Input: bulk counts + sample metadatas

▶️ Output: patient-level total counts and top genes per condition
...
▶️ Input: clinical lab values + reference ranges

▶️ Output: labs labeled normal vs out-of-range + abnormal-rate summaries per patient and per test
...
▶️ Input: lab results + vital sign measurements

▶️ Output: labs matched to nearest HR/SBP + per-patient CRP-vitals correlations
...
▶️ Input:ATAC-seq peak regions

▶️ Output: top high-scoring peaks within chr2:2--4 Mb window
...
▶️ Input: bulk counts + sample metadata

▶️ Output: genes with mean/median/Q1/Q3 counts & genes with ≥2× treated vs control expression
...
▶️ Input:wide expression matrix

▶️ Output:long format summary + wide table of mean treated vs control expression
...
▶️ Input: ATAC peaks + gene coordinates

▶️ Output: genes ranked by total ATAC overlap (bp) and peak count
...
▶️ Input: variant table + gene coordinates

▶️ Output: counts of high-impact variants per gene per sample + genes mutated in all samples
...
▶️ Input: cohort metadata A+B + bulk counts

▶️ Output: expression summary of top-variance genes per cohort × condition
...
▶️ Input: integration clusters + predicted cell types

▶️ Output: cell-type proportions per cluster split by tumor vs normal tissue
